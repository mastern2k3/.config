function mm -a 'filename' --description "MakeMeFish - List all Make targets in the Makefile of the current directory"
    ...
    function __fzf_command
        ...
        if [ $FZF_TMUX -eq 1 ]
            echo "fzf-tmux -d$FZF_TMUX_HEIGHT --preview-window right:80% --preview=\"cat Makefile | sed -n
            -E '/^{}:.*\\\$/,/^\\\$/p' | bat --color always -l makefile --plain\""
        else
            echo "fzf --preview-window right:80% --preview=\"cat Makefile | sed -n -E
            '/^{}:.*\\\$/,/^\\\$/p' | bat --color always -l makefile --plain\""
        end
    end
